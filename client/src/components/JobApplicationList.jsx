...
const JobApplicationList = ({ applications, loading, fetchError, onEdit, onDelete, onRetry }) => {
  const [filter, setFilter] = useState('All');
  const [search, setSearch] = useState('');
  const [activeCard, setActiveCard] = useState(null);
  ...
  if (fetchError) {
    return (
      <div className="list-empty">
        ...
        <p style={{ color: '#fca5a5', marginBottom: 12 }}>{fetchError}</p>
        <button className="btn-ghost" onClick={onRetry} style={{ margin: '0 auto' }}>
          ↺ Retry
        </button>
      </div>
    );
  ...
  return (
    <div className="list-section">
      ...
      {filtered.length === 0 ? (
        <div className="list-empty">
          <div className="empty-icon">{applications.length === 0 ? '📋' : '🔍'}</div>
          <p>{applications.length === 0
            ? 'No applications yet — hit "+ New Application" to track your first one!'
            : 'No results match your search or filter.'}</p>
        </div>
      ) : (
        <div className="app-grid">
          {filtered.map((app, idx) => {
