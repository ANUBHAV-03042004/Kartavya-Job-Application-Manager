...
function Card({ app, onEdit, onDelete, isEditing }) {
  const [open, setOpen] = useState(false);
  const c = S[app.status] || S.Applied;
  return (
    <div className={`app-card ${open?'expanded':''} ${isEditing?'is-editing':''}`} onClick={()=>{ if(!isEditing) setOpen(o=>!o); }}>
...
export default function JobApplicationList({ apps, loading, token, editingId, onEdit, onRefresh, onError }) {
  const [filter, setFilter] = useState('All');
  const [search, setSearch] = useState('');
  const [lastEditedId, setLastEditedId] = useState(null);

  // Track when editingId clears (edit saved) so we can float that card to top
  React.useEffect(() => {
  ...
  const shown = apps.filter(a => {
    ...
    // Float the last-edited card to the top (only when not currently editing)
    if (!editingId && lastEditedId) {
      if (a.id === lastEditedId) return -1;
      if (b.id === lastEditedId) return  1;
    }
    return 0;
  });
  ...
  return (
    <div>
      ...
      {shown.length === 0
        ...
        : <div className="grid">{shown.map(app=><Card key={app.id} app={app} onEdit={onEdit} onDelete={del} isEditing={app.id===editingId} />)}</div>
      }
    </div>
  );
}