  ...
  return (
    <div className="dash">
      ...
      <aside className={`sidebar ${sidebarOpen?'open':''}`}>
        ...
        <nav className="sb-nav">
          {navItems.map(({id,label,icon}) => (
            <button key={id} className={`sb-item ${view===id?'active':''}`}
      ...
      <div className="dash-main">
        ...
        {error && (
          <div className="err-banner">{error}<button onClick={()=>setError('')}>✕</button></div>
        )}
            ...
            <>
              <div className="view-header">
                <div>
                  ...
                  <p className="view-sub">{stats.total} total · {stats.interview} in interview · {stats.offer} offer{stats.offer!==1?'s':''}</p>
                </div>
              ...
              {showForm && (
                <div style={{marginBottom:24}}>
                  <JobApplicationForm token={token} selectedApplication={editing}
                    onCancel={()=>{ setEditing(null); setShowForm(false); }}
                    onSaved={()=>{ setEditing(null); setShowForm(false); fetchApps(); }} />
                </div>
              )}

              <JobApplicationList apps={apps} loading={loading} token={token}
                onEdit={a=>{ setEditing(a); setShowForm(true); window.scrollTo({top:0,behavior:'smooth'}); }}
                onRefresh={fetchApps} onError={setError} />
            </>
          )}
          {view==='analytics' && <Analytics apps={apps} />}
          {view==='settings'  && <Settings user={user} token={token} onLogout={onLogout} onUserUpdate={onUserUpdate} />}
        </div>
      </div>
    </div>
  );
...
const AppIcon      = () => <svg width="17" height="17" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/></svg>;
const ChartIcon    = () => <svg width="17" height="17" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/><line x1="2" y1="20" x2="22" y2="20"/></svg>;
const SettingsIcon = () => <svg width="17" height="17" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 00.33 1.82l.06.06a2 2 0 010 2.83 2 2 0 01-2.83 0l-.06-.06a1.65 1.65 0 00-1.82-.33 1.65 1.65 0 00-1 1.51V21a2 2 0 01-4 0v-.09A1.65 1.65 0 009 19.4a1.65 1.65 0 00-1.82.33l-.06.06a2 2 0 01-2.83-2.83l.06-.06A1.65 1.65 0 004.68 15a1.65 1.65 0 00-1.51-1H3a2 2 0 010-4h.09A1.65 1.65 0 004.6 9a1.65 1.65 0 00-.33-1.82l-.06-.06a2 2 0 012.83-2.83l.06.06A1.65 1.65 0 009 4.68a1.65 1.65 0 001-1.51V3a2 2 0 014 0v.09a1.65 1.65 0 001 1.51 1.65 1.65 0 001.82-.33l.06-.06a2 2 0 012.83 2.83l-.06.06A1.65 1.65 0 0019.4 9a1.65 1.65 0 001.51 1H21a2 2 0 010 4h-.09a1.65 1.65 0 00-1.51 1z"/></svg>;
const LogoutIcon   = () => <svg width="15" height="15" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path d="M9 21H5a2 2 0 01-2-2V5a2 2 0 012-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>;
const MenuIcon     = () => <svg width="20" height="20" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><line x1="3" y1="6" x2="21" y2="6"/><line x1="3" y1="12" x2="21" y2="12"/><line x1="3" y1="18" x2="21" y2="18"/></svg>;