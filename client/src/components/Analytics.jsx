...
//     <div><div className="view-header"><h1 className="view-title">Analytics</h1></div><div className="empty">
//         <div className="empty-video-wrap">
//               <video src="/analytics.mp4" autoPlay loop muted playsInline className="empty-video" />
//               <p className="empty-video-text">Add applications to see your analytics.</p>
//         </div></div></div>
//   );
...
const COLORS = [P.sky, P.lemon, P.lime, P.flame, P.forest];

function DonutChart({ data, size = 160 }) {
  const total = data.reduce((s, d) => s + d.value, 0);
  if (!total) return (
    <div style={{ width: size, height: size, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#aaa', fontSize: 13 }}>
      No data
    </div>
  );
  const r = 58, circ = 2 * Math.PI * r;
      ...
      {data.map((d, i) => {
        ...
        return (
          <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
            <div style={{ fontSize: 11, color: '#666', fontWeight: 600 }}>{d.value || ''}</div>
            <div style={{
              width: '100%', height: Math.max(bh, 2),
              background: d.color,
              borderRadius: '4px 4px 0 0',
              opacity: d.value ? 1 : 0.2,
              boxShadow: d.value ? `0 2px 8px ${d.color}88` : 'none',
...
function KPI({ label, value, color, sub }) {
  return (
    <div className="kpi-card" style={{ '--kc': color, borderTop: `3px solid ${color}` }}>
      <div className="kpi-val" style={{ color }}>{value}</div>
      <div className="kpi-label">{label}</div>
      {sub && <div className="kpi-sub">{sub}</div>}
    </div>
...
export default function Analytics({ apps }) {
  const s = useMemo(() => {
    ...
    const cm = {};
    apps.forEach(a => { cm[a.companyName] = (cm[a.companyName] || 0) + 1; });
    const top = Object.entries(cm).sort((a, b) => b[1] - a[1]).slice(0, 5);
  ...
  const donut = [
    { label: 'Applied',   value: s.by.Applied,   color: P.sky    },
    { label: 'Interview', value: s.by.Interview,  color: P.lemon  },
    { label: 'Offer',     value: s.by.Offer,      color: P.lime   },
    { label: 'Rejected',  value: s.by.Rejected,   color: P.flame  },
  ];
  ...
  return (
    <div>
      ...
      <div className="kpi-grid" style={{ marginBottom: 14 }}>
        <KPI label="Total Applied"  value={s.total}              color={P.sky}    sub="all time" />
        <KPI label="Response Rate"  value={`${s.responseRate}%`} color={P.lemon}  sub="interviews + offers" />
        <KPI label="Offer Rate"     value={`${s.offerRate}%`}    color={P.lime}   sub="of all apps" />
        <KPI label="Pending Reply"  value={s.by.Applied}         color={P.flame}  sub="awaiting response" />
      </div>

      {/* Row 2 */}
      <div className="an-row2" style={{ marginBottom: 14 }}>
        <div className="a-card">
          <div className="a-ttl">Status Breakdown</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 24, flexWrap: 'wrap' }}>
            <div style={{ position: 'relative' }}>
      ...
      <div className="an-row3">
        ...
        <div className="a-card">
          <div className="a-ttl">This Week</div>
          <div style={{ fontSize: 28, fontWeight: 800, color: P.lime, marginBottom: 4 }}>
            {s.weekly.reduce((a, b) => a + b, 0)}
          </div>
          <div style={{ fontSize: 12, color: '#888', marginBottom: 12 }}>apps in last 7 days</div>
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: 4, height: 48 }}>
            {s.weekly.map((v, i) => {
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
            {['M', 'T', 'W', 'T', 'F', 'S', 'S'].map((d, i) => (
              <div key={i} style={{ fontSize: 10, color: '#aaa' }}>{d}</div>
            ))}
          </div>
        </div>
        ...
        <div className="a-card">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginTop: 8 }}>
            {[
              { label: 'Applied',   val: s.total,                   pct: 100,  color: P.sky   },
              { label: 'Interview', val: s.by.Interview + s.by.Offer, pct: s.total ? Math.round((s.by.Interview + s.by.Offer) / s.total * 100) : 0, color: P.lemon },
              { label: 'Offer',     val: s.by.Offer,                pct: s.total ? Math.round(s.by.Offer / s.total * 100) : 0, color: P.lime  },
            ].map(f => (
              <div key={f.label}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, marginBottom: 5 }}>
                  <span style={{ color: '#555' }}>{f.label}</span>
                  <span style={{ color: f.color, fontWeight: 700 }}>
                    {f.val} <span style={{ color: '#aaa', fontWeight: 400, fontSize: 11 }}>({f.pct}%)</span>
                  </span>
                </div>
                <div style={{ height: 6, background: '#f0f0f0', borderRadius: 6, overflow: 'hidden' }}>
                  <div style={{ height: '100%', width: `${f.pct}%`, background: f.color, borderRadius: 6, transition: 'width 0.7s' }} />
            ...
            <div style={{ marginTop: 4, padding: '10px 12px', background: 'rgba(124,181,24,0.08)', borderRadius: 8, borderLeft: `3px solid ${P.lime}` }}>
              <div style={{ fontSize: 11, color: '#888' }}>Interview → Offer rate</div>
              <div style={{ fontSize: 20, fontWeight: 800, color: P.lime }}>{s.iConv}%</div>
            </div>
          </div>
        </div>
      </div>
